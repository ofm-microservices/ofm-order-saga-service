...
func (r *sessionRepository) Create(ctx context.Context, session domain.Session) (*domain.Session, error) {
	now := time.Now().UTC()
	if err := r.db.Query(insertSessionQuery, session.SagaID, session.OrderID, session.BuyerID, session.SellerID, session.BuyerEmail, session.RealtimeConnectionID, session.GigID, session.GigTitle, session.PackageID, session.PackageTier, session.PackageDescription, session.PackageDeliveryDays, session.PriceCents, session.Currency, session.Status, now, now).WithContext(ctx).Exec(); err != nil {
